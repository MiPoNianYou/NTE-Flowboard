...
describe('useNextResetLabel', () => {
  ...
  it('should use customResetMode for custom tab (weekly)', () => {
    // Wednesday June 18 → next Monday June 23 → 5 days 19h
    vi.useFakeTimers()
    vi.setSystemTime(new Date('2025-06-18T02:00:00Z'))
    const { result } = renderHook(() => useNextResetLabel({
      activeTab: 'custom',
  ...
  it('should default custom tab to daily mode', () => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date('2025-06-15T02:00:00Z')) // 10:00 UTC+8
    const { result } = renderHook(() => useNextResetLabel({
      activeTab: 'custom',
    ...
    // No customResetMode → defaults to daily → target is tomorrow 05:00 → 19h
    expect(result.current).toMatch(/\d+小时\d+分钟后重置/)
  })
