...
describe('TabSwitch', () => {
  ...
  it('should render custom tab by default', () => {
    render(<TabSwitch activeTab="daily" onTabChange={vi.fn()} />)
    expect(screen.getByText('自定义清单')).toBeInTheDocument()
  })

  it('should hide custom tab when showCustomTab is false', () => {
    render(<TabSwitch activeTab="daily" onTabChange={vi.fn()} showCustomTab={false} />)
    expect(screen.queryByText('自定义清单')).not.toBeInTheDocument()
  })
  ...
  it('should display customName when provided', () => {
    render(<TabSwitch activeTab="custom" onTabChange={vi.fn()} customName="我的清单" />)
    expect(screen.getByText('我的清单')).toBeInTheDocument()
  })
})
