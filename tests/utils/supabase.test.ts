...
describe('SyncError', () => {
  it('should be an instance of Error', () => {
    const err = new SyncError('test', 'CODE')
...
describe('loadSupabaseConfig', () => {
  ...
  it('should load legacy format config (pid/key)', () => {
    localStorage.setItem(CONFIG_KEY, JSON.stringify({ pid: 'legacy-p', key: 'legacy-k' }))
    const config = loadSupabaseConfig()
    expect(config).toEqual({ projectId: 'legacy-p', anonKey: 'legacy-k' })
  })
  ...
  it('should return null for legacy with empty pid', () => {
    ...
    const config = loadSupabaseConfig()
    expect(config).toEqual({ projectId: '', anonKey: 'k' })
  })
})
...
describe('pullData', () => {
  const validData = JSON.stringify({ daily: [], weekly: [], custom: [], resetConfig: { serverRegion: 'asia' }, lastDailyReset: '', lastWeeklyReset: '' })
