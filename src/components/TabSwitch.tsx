...
export const TabSwitch = memo(function TabSwitch({ activeTab, onTabChange, customName }: TabSwitchProps) {
  return (
    <div className="flex flex-col gap-2" role="tablist">
      {/* 第一行：每日 + 每周 */}
      <div className="flex gap-1 p-1 bg-surface rounded-full border border-border">
        {tabs.map(({ key, label, icon: Icon }) => (
          <button
            ...
            className={cn(
              'flex-1 flex items-center justify-center gap-2.5 py-2.5 rounded-full text-base font-medium relative z-10 transition-colors duration-150',
              activeTab === key
                ? 'text-text-primary'
