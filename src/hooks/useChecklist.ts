...
import type { ChecklistData, CustomResetMode, ResetConfig, TabType } from '../types'
...
function applyReset(data: ChecklistData): ChecklistData {
  ...
  if (shouldResetCustom(data)) {
    next.custom = resetItems(data.custom)
    next.lastCustomReset = new Date().toISOString()
    needsUpdate = true
  }
...
export function useChecklist() {
  ...
  const setCustomResetMode = useCallback((mode: CustomResetMode) => {
    setData((prev) => ({
      ...prev,
      customResetMode: mode,
      lastCustomReset: new Date().toISOString(),
    }))
  }, [])
